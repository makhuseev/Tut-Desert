...
const escHtml=s=>String(s??"").replace(
  /[&<>"']/g,
  m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#039;"}[m])
);
...
function applySiteSettings(){
  ...
  if(addressEl) addressEl.textContent=s.address||DEFAULT_SITE_SETTINGS.address;

  const phoneEl=document.getElementById("sitePhone");
  if(phoneEl){
    phoneEl.textContent=s.phone||DEFAULT_SITE_SETTINGS.phone;
    phoneEl.href="https://wa.me/"+(s.whatsapp||DEFAULT_SITE_SETTINGS.whatsapp);
  }

  const mapEl=document.getElementById("siteMap");
  if(mapEl) mapEl.href=s.map_url||DEFAULT_SITE_SETTINGS.map_url;

  const hoursEl=document.getElementById("siteHours");
  if(hoursEl) hoursEl.textContent=s.hours||DEFAULT_SITE_SETTINGS.hours;

  const waButtons=document.querySelectorAll("[data-whatsapp]");
  waButtons.forEach(a=>{
    const num=s.whatsapp||DEFAULT_SITE_SETTINGS.whatsapp;
    const msg=a.dataset.whatsappMessage||"Здравствуйте! Хочу сделать заказ в Tut Dessert.";
  ...
  if(s.logo_url){
    logoLinks.forEach(a=>{
      a.innerHTML=`<img src="${escHtml(s.logo_url)}" alt="Tut Dessert" style="max-height:42px;width:auto;object-fit:contain;">`;
    });
  }
}
...
async function loadSiteSettings(){
  ...
  if(!error && data){
    SITE_SETTINGS={...DEFAULT_SITE_SETTINGS,...data};
  }
...
async function loadProducts(){
  ...
  const {data,error}=await supabaseClient
    .from("products")
    .select("id,name,category,price,price_label,unit,description,emoji,image_url,is_active")
    .eq("is_active",true)
  ...
  document.getElementById("productGrid").innerHTML=list.map(p=>`
    <article class="product-card">
      ...
      <div class="product-art"
        ${p.image_url
          ?`onclick="openPhotoModal('${p.image_url}','${p.name}')"`
          :""
        }>
...
function saveCart(){
  localStorage.setItem("tutCart",JSON.stringify(cart));
  renderCartCount();
}
...
function renderCart(){
    ...
    rows.length
      ?rows.map(p=>`
        <div class="cart-row">
          <div>
            <h4>${p.name}</h4>
            <small>${displayPrice(p)} ${p.unit||""}</small>
          </div>
  ...
  const total=rows.reduce(
    (s,p)=>s+(typeof p.price==="number"?p.price*p.qty:0),
    0
  );

  document.getElementById("cartTotal").textContent=
    money(total)+
    (rows.some(p=>typeof p.price!=="number")
      ?" + товары с ценой по размеру"
      :"");
}
...
document.getElementById("orderCart").onclick=()=>{
  ...
  if(!text){
    return alert("Добавьте товары в корзину.");
  }
  ...
  window.open(
    ...
    "?text="+encodeURIComponent(
      "Здравствуйте! Хочу сделать заказ в Tut Dessert:\n"+
      text+
      "\n\nПодскажите, пожалуйста, итоговую стоимость."
    ),
    "_blank"
  );
};
